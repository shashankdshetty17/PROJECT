...
def get_product_names(soup_object: BeautifulSoup):
    ...
    prname = soup_object.find('span', {'class': 'a-size-large product-title-word-break'}).get_text().strip()
    return prname

def get_product_category(soup_object: BeautifulSoup):
    #(//span[@class='nav-a-content'])[1]
    try:
        cat = soup_object.find('span', {'class': 'a-size-base-plus a-color-base a-text-bold'}).get_text().strip()
        return cat
    except:
