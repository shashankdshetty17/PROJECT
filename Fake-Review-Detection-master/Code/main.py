...
def feature_engineering(df):
    ...
    bag_of_words = vectorizer.fit_transform(df['text_'])
    # Convert the sparse matrix to a DataFrame
    bow_df = pd.DataFrame(bag_of_words.toarray(), columns=vectorizer.get_feature_names_out())
    
    # Apply dimensionality reduction to reduce the number of features
    n_components = 1000  # Number of components to keep
    svd = TruncatedSVD(n_components=n_components)
    bow_reduced = svd.fit_transform(bow_df)

    # Create a new DataFrame with the reduced features
    bow_reduced_df = pd.DataFrame(bow_reduced, columns=[f"component_{i+1}" for i in range(n_components)])

    # Print the DataFrame
    print(bow_reduced_df)

    ########
    #ndf = pd.concat([bow_df, df], axis=1)
